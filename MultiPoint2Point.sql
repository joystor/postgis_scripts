...
BEGIN
 FOR _recgeoms IN
    select '"'||f_table_schema||'"."'||f_table_name||'"' tablename, f_geometry_column geomcolumn from geometry_columns where type='MULTIPOINT'
 LOOP
   ...
   IF _rcount.multi_geom = 0 THEN
     ...
     _msg = 'ALTER TABLE ' || _recgeoms.tablename || ' ALTER COLUMN '||_recgeoms.geomcolumn||' TYPE geometry(Point,4326) USING ST_GeometryN('||_recgeoms.geomcolumn||', 1)';
   ELSE
